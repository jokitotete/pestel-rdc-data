...
export default function App() {
  ...
  const toggleNotif = useCallback(async () => {
    ...
    // Un seul message pour TOUTES les causes d'échec (permission refusée, notifications coupées au niveau
    // système, module natif absent d'un build) : l'utilisateur n'a qu'un seul geste utile à faire, et lui
    // détailler la cause technique ne l'aiderait pas. L'app ne plante jamais, elle DIT que ça n'a pas pris.
    if (!ok) Alert.alert('Briefing non activé', "Le briefing du matin n'a pas pu être programmé. Vérifiez que les notifications de Ntongo sont autorisées dans les réglages de votre téléphone.");
  }, [notifOn]);
  ...
  useEffect(() => {
    ...
    loadPrefs().then((p) => {                                                  // hydrate dernière édition vue + opt-in notif
      ...
      // RÉCONCILIATION avec l'OS (anti-ARCA appliqué à l'UI) : notre préférence dit « activé », mais
      // l'utilisateur a pu révoquer la permission depuis les réglages système — la cloche allumée
      // affirmerait alors une chose FAUSSE. On affiche d'abord l'état persisté (pas de clignotement),
      // puis on ne l'ÉTEINT que sur un démenti FRANC de l'OS (`false`) ; `null` = indéterminé = on ne
      // touche à rien (voir notify.js, tri-état assumé).
      if (p.notifOn) {
        setNotifOn(true);
        isBriefingScheduled().then((vif) => {
          if (vif === false) { setNotifOn(false); savePrefs({ notifOn: false }); }
        });
      }
